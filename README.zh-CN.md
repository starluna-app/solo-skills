...
`solo-skills` 提供了一系列专为独立创业者设计的 AI 自动化技能。无论是竞品调研、跨平台文案生成，还是 YouTube 频道策划，你都可以通过终端直接调用，或者将它们无缝集成到你最喜欢的 AI IDE（如 Cursor、Windsurf）和 AI 助手（如 Claude Code、Gemini CLI）中。

## ✨ 核心特性

* **⚡️ 零配置运行**：无需繁琐的安装，直接通过 `npx` 即可在终端随时唤起。
* **🌍 国际化 (i18n) 支持**：默认英文，大部分技能提供中文（`zh-CN`）版本，AI Agent 会根据你的对话上下文自动选择语言。
